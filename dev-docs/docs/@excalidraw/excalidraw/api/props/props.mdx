...
| [`imageContextMenuItems`](#imagecontextmenuitems) | `function` | \_ | Allows the host to inject custom context menu items for all-image selections. |
...
Use this prop to inject custom context menu items when the current selection contains only image elements.
...
The callback receives the selected image element ids, not file ids. This keeps the menu generation cheap and lets the host resolve file data through the Excalidraw API if needed.

Returned items are rendered below the built-in cut, copy, and paste actions, and above wrap in frame.
