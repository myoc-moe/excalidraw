...
| [scrollToViewport](#scrolltoviewport) | `function` | Scroll the viewport to an exact `scrollX`/`scrollY`/`zoom` target. |
...
Scroll to an exact saved viewport target.

| Attribute | type | default | Description |
| --- | --- | --- | --- |
| target.scrollX | number | \_ | Exact horizontal scroll target. |
| target.scrollY | number | \_ | Exact vertical scroll target. |
| target.zoom | number | \_ | Exact zoom target. The value is normalized to the supported zoom range before being applied. |
| opts.animate | boolean | false | Whether to animate between starting and ending viewport values. |
