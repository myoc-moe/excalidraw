...
export function curveToBezier(pointsIn, curveTightness = 0) {
    ...
    if (len === 3) {
        ...
        for (let i = 1; i + 2 < points.length; i++) {
            ...
            b[1] = pointFrom(cachedVertArray[0] + (s * points[i + 1][0] - s * points[i - 1][0]) / 6, cachedVertArray[1] + (s * points[i + 1][1] - s * points[i - 1][1]) / 6);
            b[2] = pointFrom(points[i + 1][0] + (s * points[i][0] - s * points[i + 2][0]) / 6, points[i + 1][1] + (s * points[i][1] - s * points[i + 2][1]) / 6);
