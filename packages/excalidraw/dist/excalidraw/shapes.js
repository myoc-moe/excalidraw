import { isPoint, pointFrom, pointDistance, pointFromPair, pointRotateRads, pointsEqual, } from "../math";
import { getClosedCurveShape, getCurvePathOps, getCurveShape, getEllipseShape, getFreedrawShape, getPolygonShape, } from "../utils/geometry/shape";
    ...
    switch (element.type) {
        ...
        case "line": {
            ...
            return shouldTestInside(element)
                ? getClosedCurveShape(element, roughShape, pointFrom(element.x, element.y), element.angle, pointFrom(cx, cy))
                : getCurveShape(roughShape, pointFrom(element.x, element.y), element.angle, pointFrom(cx, cy));
        }
        case "ellipse":
...
export const getControlPointsForBezierCurve = (element, endPoint) => {
    ...
    while (index < ops.length) {
        ...
        if (op === "move") {
            invariant(isPoint(data), "The returned ops is not compatible with a point");
...
const getPointsInBezierCurve = (element, endPoint) => {
    ...
    while (t > 0) {
        const p = getBezierXY(controlPoints[0], controlPoints[1], controlPoints[2], controlPoints[3], t);
...
export const aabbForElement = (element, offset) => {
    ...
    const [topLeftX, topLeftY] = pointRotateRads(pointFrom(bbox.minX, bbox.minY), center, element.angle);
    const [topRightX, topRightY] = pointRotateRads(pointFrom(bbox.maxX, bbox.minY), center, element.angle);
    const [bottomRightX, bottomRightY] = pointRotateRads(pointFrom(bbox.maxX, bbox.maxY), center, element.angle);
    const [bottomLeftX, bottomLeftY] = pointRotateRads(pointFrom(bbox.minX, bbox.maxY), center, element.angle);
    const bounds = [
...
export const aabbsOverlapping = (a, b) => pointInsideBounds(pointFrom(a[0], a[1]), b) ||
