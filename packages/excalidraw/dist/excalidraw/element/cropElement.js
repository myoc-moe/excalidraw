...
export const cropElement = (element, transformHandle, naturalWidth, naturalHeight, pointerX, pointerY, widthAspectRatio) => {
     ...
     * *––––––––––––––––––––––––*
     */
    const rotatedPointer = pointRotateRads(pointFrom(pointerX, pointerY), pointFrom(element.x + element.width / 2, element.y + element.height / 2), -element.angle);
    ...
    if (transformHandle.includes("n")) {
        nextHeight = clamp(element.height - changeInHeight, MINIMAL_CROP_SIZE, isFlippedByY ? uncroppedHeight - croppedTop : element.height + croppedTop);
    }
    if (transformHandle.includes("s")) {
        changeInHeight = pointerY - element.y - element.height;
        nextHeight = clamp(element.height + changeInHeight, MINIMAL_CROP_SIZE, isFlippedByY ? element.height + croppedTop : uncroppedHeight - croppedTop);
    }
    if (transformHandle.includes("e")) {
        changeInWidth = pointerX - element.x - element.width;
        nextWidth = clamp(element.width + changeInWidth, MINIMAL_CROP_SIZE, isFlippedByX ? element.width + croppedLeft : uncroppedWidth - croppedLeft);
    }
    if (transformHandle.includes("w")) {
        nextWidth = clamp(element.width - changeInWidth, MINIMAL_CROP_SIZE, isFlippedByX ? uncroppedWidth - croppedLeft : element.width + croppedLeft);
    }
    ...
    if (isCloseTo(crop.width, crop.naturalWidth) &&
...
const recomputeOrigin = (stateAtCropStart, transformHandle, width, height, shouldMaintainAspectRatio) => {
    const [x1, y1, x2, y2] = getResizedElementAbsoluteCoords(stateAtCropStart, stateAtCropStart.width, stateAtCropStart.height, true);
...
export const getUncroppedImageElement = (element, elementsMap) => {
    if (element.crop) {
        ...
        const topLeftVector = vectorFromPoint(pointRotateRads(pointFrom(x1, y1), pointFrom(cx, cy), element.angle));
        const topRightVector = vectorFromPoint(pointRotateRads(pointFrom(x2, y1), pointFrom(cx, cy), element.angle));
        const topEdgeNormalized = vectorNormalize(vectorSubtract(topRightVector, topLeftVector));
        const bottomLeftVector = vectorFromPoint(pointRotateRads(pointFrom(x1, y2), pointFrom(cx, cy), element.angle));
        ...
        const rotatedTopLeft = vectorAdd(vectorAdd(topLeftVector, vectorScale(topEdgeNormalized, (-cropX * width) / element.crop.naturalWidth)), vectorScale(leftEdgeNormalized, (-cropY * height) / element.crop.naturalHeight));
        const center = pointFromVector(vectorAdd(vectorAdd(rotatedTopLeft, vectorScale(topEdgeNormalized, width / 2)), vectorScale(leftEdgeNormalized, height / 2)));
        const unrotatedTopLeft = pointRotateRads(pointFromVector(rotatedTopLeft), center, -element.angle);
...
export const getUncroppedWidthAndHeight = (element) => {
    if (element.crop) {
