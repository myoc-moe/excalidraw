...
Break.Before(COMMON.WHITESPACE).Build(), Break.After(COMMON.WHITESPACE, COMMON.HYPHEN).Build(), 
// Rules for CJK (chars, symbols, currency)
Break.Before(CJK.CHAR, CJK.CURRENCY)
    .NotPrecededBy(COMMON.OPENING, CJK.OPENING)
    .Build(), Break.After(CJK.CHAR)
...
const isSingleCharacter = (maybeSingleCharacter) => {
    return (maybeSingleCharacter.codePointAt(0) !== undefined &&
