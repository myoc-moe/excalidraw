import { HEADING_DOWN, HEADING_LEFT, HEADING_RIGHT, HEADING_UP, compareHeading, headingForPointFromElement, } from "./heading";
...
export const getPredecessors = (node, elementsMap, direction) => {
...
export const addNewNodes = (startNode, elementsMap, appState, direction, numberOfNodes) => {
...
const createBindingArrow = (startBindingElement, endBindingElement, elementsMap, direction, appState) => {
    ...
    bindLinearElement(bindingArrow, startBindingElement, "start", elementsMap);
    bindLinearElement(bindingArrow, endBindingElement, "end", elementsMap);
...
export class FlowChartCreator {
    ...
    createNodes(startNode, elementsMap, appState, direction) {
        if (direction !== this.direction) {
            const { nextNode, bindingArrow } = addNewNode(startNode, elementsMap, appState, direction);
        ...
        if (startNode.frameId) {
            ...
            if (frame &&
                this.pendingNodes.every((node) => elementsAreInFrameBounds([node], frame, elementsMap) ||
                    elementOverlapsWithFrame(node, frame, elementsMap))) {
                this.pendingNodes = this.pendingNodes.map((node) => mutateElement(node, {
...
export const isNodeInFlowchart = (element, elementsMap) => {
