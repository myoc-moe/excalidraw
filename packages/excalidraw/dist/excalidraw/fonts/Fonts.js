...
import { FONT_METADATA } from "./FontMetadata";
export class Fonts {
    ...
    static async generateFontFaceDeclarations(elements) {
        ...
        const familyWithCJK = families.find((x) => getFontFamilyFallbacks(x).includes(CJK_HAND_DRAWN_FALLBACK_FONT));
        if (familyWithCJK) {
    ...
    static register(family, metadata, ...fontFacesDecriptors) {
        ...
        const fontFamily = FONT_FAMILY[family] ??
            FONT_FAMILY_FALLBACKS[family];
        const registeredFamily = this.registered.get(fontFamily);
        if (!registeredFamily) {
            this.registered.set(fontFamily, {
                metadata,
                fontFaces: fontFacesDecriptors.map(({ uri, descriptors }) => new ExcalidrawFontFace(family, uri, descriptors)),
            });
        }
